...
var ASM_CONSTS = {
  6348160: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 6348326: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 6348375: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 6348424: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 6348524: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 6348718: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 6348814: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 6349345: ($0) => { var adtype = UTF8ToString($0); const callbacks = ({ adFinished: () => ccall('crazygamesad_finished_callback', null), adError: (error) => ccall('crazygamesad_error_callback', null, ['string'], [error ? error.message : ""]), adStarted: () => ccall('crazygamesad_started_callback', null) }); window.CrazyGames.SDK.ad.requestAd(adtype, callbacks); },  
 6349693: () => { window.CrazyGames.SDK.game.happytime(); },  
 6349733: () => { window.CrazyGames.SDK.game.gameplayStart(); },  
 6349777: () => { window.CrazyGames.SDK.game.gameplayStop(); },  
 6349820: () => { window.CrazyGames.SDK.game.sdkGameLoadingStart(); },  
 6349870: () => { window.CrazyGames.SDK.game.sdkGameLoadingStop(); },  
