...
// end include: /tmp/tmpe3yil5b5.js
// include: /tmp/tmp79pejql2.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if (Module['$ww'] || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmp79pejql2.js
// include: /tmp/tmpocxpyezo.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpocxpyezo.js
...
var ASM_CONSTS = {
  6347216: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 6347382: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 6347431: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 6347480: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 6347580: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 6347774: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 6347870: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 6348401: ($0) => { var adtype = UTF8ToString($0); const callbacks = ({ adFinished: () => ccall('crazygamesad_finished_callback', null), adError: (error) => ccall('crazygamesad_error_callback', null, ['string'], [error ? error.message : ""]), adStarted: () => ccall('crazygamesad_started_callback', null) }); window.CrazyGames.SDK.ad.requestAd(adtype, callbacks); },  
 6348749: () => { window.CrazyGames.SDK.game.happytime(); },  
 6348789: () => { window.CrazyGames.SDK.game.gameplayStart(); },  
 6348833: () => { window.CrazyGames.SDK.game.gameplayStop(); },  
 6348876: () => { window.CrazyGames.SDK.game.sdkGameLoadingStart(); },  
 6348926: () => { window.CrazyGames.SDK.game.sdkGameLoadingStop(); },  
